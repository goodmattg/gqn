...
from model.gqn_encoder import pool_encoder
...
context_poses = tf.placeholder(
    shape=[_BATCH_SIZE, _CONTEXT_SIZE, _DIM_POSE], dtype=tf.float32
)
...
r_encoder_batch = tf.reshape(
    r_encoder_batch, shape=[_BATCH_SIZE, _CONTEXT_SIZE, 1, 1, _DIM_C_ENC]
)  # 1, 1 for pool encoder only!
r_encoder = tf.reduce_sum(
    r_encoder_batch, axis=1
