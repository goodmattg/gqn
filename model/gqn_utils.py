...
def create_sub_scope(scope, name):
    # TODO(stefan): add docstring
    if scope is None:
        varscope = tf.compat.v1.get_variable_scope()
    else:
        varscope = tf.compat.v1.variable_scope(scope)

    with varscope, tf.compat.v1.variable_scope(name) as subvarscope:
        return subvarscope
...
def eta_g(
    ...
    return tf.compat.v1.layers.conv2d(
        canvas, filters=channels, kernel_size=kernel_size, padding="SAME", name=scope
    )
...
def eta(
    ...
    channels=GQN_DEFAULT_CONFIG.Z_CHANNELS,
):
    """
  ...
  """
    ...
    eta = tf.compat.v1.layers.conv2d(  # 2 * channels because mu and sigma need to be computed per channel
    ...
    sigma = tf.nn.softplus(sigma + 0.5) + 1e-8  # TODO(ogroth): check

    return mu, sigma


@optional_scope
def compute_eta_and_sample_z(
    h,
    kernel_size=GQN_DEFAULT_CONFIG.ETA_INTERNAL_KERNEL_SIZE,
    channels=GQN_DEFAULT_CONFIG.Z_CHANNELS,
):
    """
  ...
  """
    ...
    with tf.compat.v1.variable_scope("Sampling"):
        z_shape = tf.concat(
            [tf.shape(input=h)[:-1], [channels]], axis=0, name="CreateZShape"
        )
        z = mu + tf.multiply(sigma, tf.random.normal(shape=z_shape))
...
def debug_canvas_image_mean(canvases, eta_g_scope="GQN"):
  ...
  """
    with tf.compat.v1.variable_scope(
        eta_g_scope, reuse=True, auxiliary_name_scope=False
    ), tf.compat.v1.name_scope("debug"):
        mean_images = []

        with tf.compat.v1.name_scope("MakeWhiteBar"):
            cs = tf.shape(input=canvases[0])
