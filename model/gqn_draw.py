...
from .gqn_utils import (
    ...
    create_sub_scope,
    eta_g,
    compute_eta_and_sample_z,
    sample_z,
)


class GQNLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    """
        ...
        self._state_size = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(
    ...
    """
        ...
        new_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(new_cell, output)
...
_GeneratorCellInput = namedtuple(
    "GeneratorCellInput", ["representation", "query_pose", "z"]
)
_GeneratorCellOutput = namedtuple(
    "GeneratorCellOutput", ["canvas", "lstm"]
)  # canvas, h
_GeneratorCellState = namedtuple(
    "GeneratorCellState", ["canvas", "lstm"]
)  # canvas, (c, h)


class GeneratorLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    # TODO(stefan): better description here
    """GeneratorLSTMCell wrapper that upscales output with a deconvolution and
     adds it to a side input."""
    ...
    """
        ...
        self._output_size = _GeneratorCellOutput(
            canvas_size, self._gqn_cell.output_size
        )
    ...
    def call(
        self, inputs: _GeneratorCellInput, state: _GeneratorCellState, scope=None
    ) -> _GeneratorCellOutput:
        """
    ...
    """
        ...
        sub_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(cell_state, hidden_state)
        ...
        new_canvas = canvas + tf.compat.v1.layers.conv2d_transpose(
            sub_output,
            filters=self._canvas_channels,
            kernel_size=4,
            strides=4,
...
_InferenceCellInput = namedtuple(
    "InferenceCellInput",
    ["representation", "query_pose", "query_image", "canvas", "h_g"],
)
_InferenceCellOutxput = namedtuple("InferenceCellOutput", ["lstm"])  # h
_InferenceCellState = namedtuple("InferenceCellOutput", ["lstm"])  # c, h


class InferenceLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    # TODO(stefan): better description here
    ...
    def call(
        self, inputs: _InferenceCellInput, state: _InferenceCellState, scope=None
    ) -> _InferenceCellOutput:
        """
    :param inputs: concatenated pose, representation, and noise z
    :param state:
    :param scope:
    ...
    """
        ...
        input_canvas_and_image = tf.compat.v1.layers.conv2d(
            tf.concat([query_image, canvas], axis=-1),
            filters=self.output_size.lstm[-1],
            kernel_size=4,
            strides=4,
            padding="VALID",
            use_bias=False,
        ...
        state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(cell_state, hidden_state)
...
def generator_rnn(representations, query_poses, params, scope="GQN_RNN"):
    """
  ...
  """
    ...
    batch = tf.shape(input=representations)[0]
    ...
    outputs = []
    endpoints = {}
    with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE) as varscope:
...
def inference_rnn(representations, query_poses, target_frames, params, scope="GQN_RNN"):
    """
    ...
    with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE) as varscope:
        ...
        for step in range(params.SEQ_LENGTH):
            ...
            with tf.compat.v1.name_scope("Inference"):
                (inf_output, inf_state) = inference_cell(
                    inf_input, inf_state, "LSTM_inf"
                )
            # estimate statistics and sample state from posterior
            mu_q, sigma_q, z_q = compute_eta_and_sample_z(
                inf_state.lstm.h, scope="Sample_eta_q"
            )
            # input into generator RNN
            gen_input = _GeneratorCellInput(representations, query_poses, z_q)
            # update generator cell
            with tf.compat.v1.name_scope("Generator"):
                (gen_output, gen_state) = generator_cell(
                    gen_input, gen_state, "LSTM_gen"
                )
            # estimate statistics of prior for KL divergence
            mu_pi, sigma_pi, z_pi = compute_eta_and_sample_z(
                gen_state.lstm.h, scope="Sample_eta_pi"
            )
            # aggregate outputs
